...
- **An explicit styling philosophy, documented in the app itself.** The visual system is one named choice for every decision — one accent color, one type scale, one radius scale, one elevation scale, one set of typography utilities — and nothing in the product is allowed to reach outside it. To make that enforceable rather than aspirational, the rules live at `/styleguide` as a live, rendered specification: every token, every scale, every type style shown next to the components that consume it. The route is not part of the user-facing product — it's the style guide as documentation, kept inside the codebase so the spec and the implementation can't drift apart. If a value isn't on that page, it shouldn't appear in the product.
...
8. **`/styleguide`.** Not a user feature — this route is the styling philosophy documented in the codebase. Every token, scale, and rule rendered live (colors, radii, shadows, typography, the components themselves) so the spec and the implementation can't drift apart. Worth a look to see how tightly the visual system is constrained.
